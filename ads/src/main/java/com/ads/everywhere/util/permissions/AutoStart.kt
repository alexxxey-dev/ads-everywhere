...
class AutoStart {
    ...
    fun autoStartBruteforce(context: Context, open: Boolean, newTask: Boolean): Boolean {
        return autoStart(
            context,
            listOf(
                PACKAGE_ASUS_MAIN,
            ...
            open
        ) || launchOppoAppInfo(context, open, newTask)
    }
     ...
     */
    fun getIntent(packageName: String, componentName: String, newTask: Boolean): Intent {
        return Intent().apply {
     ...
     * @return the intent generated
     */
    fun getIntentFromAction(intentAction: String, newTask: Boolean): Intent {
        return Intent().apply {
            action = intentAction
            if (newTask) addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
        }
     ...
     */
    fun isActivityFound(context: Context, intent: Intent): Boolean {
    ...
    private fun autoStart(
        ...
        return if (packages.any { isPackageExists(context, it) }) {
            if (open) {
                ...
                areActivitiesFound(context, intents)
            }
        } else false
    }
